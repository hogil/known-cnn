#!/bin/bash
# 260510 — Phase 27: re-run stage1 at n_per_class=200 for NEW HEADLINE 4-bag components
# Goal: 3x more eval chips for tighter confidence on 0.9992 number
set -e
cd /d/project/known-cnn

V15="D:/project/data/wm-811k/chip_multilabel_v15direct"
RUN_LOG=outputs/_phase27_n200_eval.log

echo "$(date) [phase27] start n=200 eval for NEW HEADLINE components" > "$RUN_LOG"
...
eval_one() {
    ...
    if [ -z "$MODEL" ]; then
        echo "$(date) [phase27-${TAG}] MODEL NOT FOUND: $MODEL_PATTERN" >> "$RUN_LOG"
        return 0
    fi
    OUT_DIR=$(dirname "$MODEL")/eval_v15direct_n200
    echo "$(date) [phase27-${TAG}] eval $MODEL" >> "$RUN_LOG"
    set +e
    python -m chip_multilabel.run_stage1 \
        ...
        >> "$RUN_LOG" 2>&1
    echo "$(date) [phase27-${TAG}] DONE" >> "$RUN_LOG"
    set -e
}
...
echo "$(date) [phase27] DONE" >> "$RUN_LOG"
