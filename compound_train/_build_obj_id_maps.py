...
PROB_OUT_ROOT = Path("D:/project/data/wm-811k/obj_prob_maps")
...
def collect_wafer_specs(
    limit_per_class: Optional[int],
    out_root: Path = OUT_ROOT,
    prob_out_root: Optional[Path] = None,
    ...
    if active_classes is not None:
        ...
        present = {p.name for p in cls_dirs}
        missing = [c for c in active_classes if c not in present]
        if missing and not allow_missing_active:
            raise SystemExit(f"missing active classes (set --allow-missing-active-classes "
                             f"to skip): {missing}")
        cls_dirs = [p for p in cls_dirs if p.name in active_set]
    for cls_dir in cls_dirs:
        ...
        for png_path in pngs:
            ...
            specs.append({
                ...
                "out_prob_npy": str(prob_out_root / subfolder / f"{basename}.npy") if prob_out_root else None,
                "grid_w": grid_w,
                "grid_h": grid_h,
                "chips": chip_entries,
            })
...
def main() -> int:
    ...
    ap.add_argument("--save-prob-maps", action="store_true",
                    help="V4 chipgrid용 per-chip softmax 확률맵을 별도 root에 저장")
    ap.add_argument("--prob-out-root", default=str(PROB_OUT_ROOT),
                    help="--save-prob-maps 출력 root (default: D:/project/data/wm-811k/obj_prob_maps)")
    ap.add_argument("--prob-dtype", default="float16", choices=["float16", "float32"],
                    help="probability map 저장 dtype (default float16)")
    ...
    ap.add_argument("--active-classes-yaml", default=None,
                    help="YAML with `classes:` list — only process those wafer classes")
    ap.add_argument("--allow-missing-active-classes", action="store_true",
                    help="don't error when active YAML class missing from PNG_ROOT")
    args = ap.parse_args()

    active_classes = None
    if args.active_classes_yaml:
        import yaml as _yaml
        with open(args.active_classes_yaml, "r", encoding="utf-8") as fy:
            _doc = _yaml.safe_load(fy) or {}
        active_classes = list(_doc.get("classes") or [])
        print(f"[active-classes] {len(active_classes)} from {args.active_classes_yaml}", flush=True)
    ...
    prob_out_root = Path(args.prob_out_root) if args.save_prob_maps else None
    if prob_out_root is not None:
        prob_out_root.mkdir(parents=True, exist_ok=True)
        print(f"[prob-maps] save enabled root={prob_out_root} dtype={args.prob_dtype}", flush=True)
    ...
    specs, skipped = collect_wafer_specs(args.limit_per_class, out_root=out_root, prob_out_root=prob_out_root,
                                          active_classes=active_classes,
                                          allow_missing_active=args.allow_missing_active_classes)
    print(f"[scan] wafers={len(specs)} skipped={skipped}", flush=True)

    if not args.overwrite:
        before = len(specs)
        def required_outputs_done(s: dict) -> bool:
            hard_done = Path(s["out_npy"]).exists()
            prob_done = (not args.save_prob_maps) or Path(str(s["out_prob_npy"])).exists()
            return hard_done and prob_done
        specs = [s for s in specs if not required_outputs_done(s)]
    ...
    with torch.no_grad():
        for spec_i, spec in enumerate(specs):
            ...
            prob_map = (
                np.zeros((n_chip_objects, grid_h, grid_w), dtype=np.float32)
                if args.save_prob_maps else None
            )
            wafer_class = spec["wafer_class"]
            basename = spec["basename"]
            parts = spec["basename_parts"]
            out_npy = Path(spec["out_npy"])
            hard_exists_before = out_npy.exists()
            write_hard_outputs = args.overwrite or not hard_exists_before
            ...
            for i in range(0, len(chips), args.batch):
                ...
                for c, p, cf, prob_vec, crop_pil in zip(batch_chips, preds, confs, probs_np, crops_pil):
                    oid = int(class_idx_to_obj_id[int(p)])
                    obj_map[c["gy"], c["gx"]] = oid
                    if prob_map is not None:
                        prob_map[:, c["gy"], c["gx"]] = prob_vec
            ...
            # save hard obj_id map only when it is missing or overwrite was requested.
            # This lets V4 probability maps be backfilled without touching existing hard maps.
            if write_hard_outputs:
                out_npy.parent.mkdir(parents=True, exist_ok=True)
                np.save(out_npy, obj_map)
            if prob_map is not None:
                out_prob_npy = Path(str(spec["out_prob_npy"]))
                out_prob_npy.parent.mkdir(parents=True, exist_ok=True)
                prob_dtype = np.float16 if args.prob_dtype == "float16" else np.float32
                np.save(out_prob_npy, prob_map.astype(prob_dtype, copy=False))
            # save PNG
            if args.save_png and write_hard_outputs:
                try:
    ...
    meta = {
        ...
        "prob_out_root": str(prob_out_root).replace("\\", "/") if prob_out_root is not None else None,
    ...
    if prob_out_root is not None:
        prob_meta_path = prob_out_root / "_meta.json"
        prob_meta = {
            "n_chip_objects": n_chip_objects,
            "chip_classes": list(classes),
            "obj_id_to_label": obj_id_to_label,
            "chip_model": str(chip_ckpt).replace("\\", "/"),
            "source_obj_id_root": str(out_root).replace("\\", "/"),
            "shape": "[n_chip_objects, grid_h, grid_w]",
            "dtype": args.prob_dtype,
            "min_defect_bin": MIN_DEFECT_BIN,
            "built_at": time.strftime("%Y-%m-%d %H:%M:%S"),
        }
        with prob_meta_path.open("w", encoding="utf-8") as f:
            json.dump(prob_meta, f, indent=2, ensure_ascii=False)
        print(f"[prob-meta] {prob_meta_path}", flush=True)
    return 0
