...
object type id:
  ...
  1 = invalid_main
  2 = scratch
  3 = fork                                                                              (round 26: was particle_blast)
  4 = scratch_rot                                                                       (round 26: was scratch_21deg)
  5 = bank_boundary
...
_ACTIVE_CLASSES = None    # if set (list), restricts FilteredImageFolder to those only


def set_active_classes(classes_list):
    """학습 entry 의 main() 에서 호출하여 active class 제한 (yaml 의 list)."""
    global _ACTIVE_CLASSES
    _ACTIVE_CLASSES = list(classes_list) if classes_list else None


class FilteredImageFolder(ImageFolder):
    """ImageFolder + EXCLUDE_CLASSES 필터 + (optional) ACTIVE_CLASSES 제한."""
    def find_classes(self, directory):
        classes, _ = super().find_classes(directory)
        kept = [c for c in classes if c not in EXCLUDE_CLASSES]
        if _ACTIVE_CLASSES is not None:
            active_set = set(_ACTIVE_CLASSES)
            # active 의 list 순서 보존 (yaml 의 순서 유지)
            kept = [c for c in _ACTIVE_CLASSES if c in set(kept)]
...
class FailObjImageFolder(FilteredImageFolder):
    ...
    def __init__(self, root: str, obj_id_root: str, img_size: int, train_aug: bool,
                 n_chip_objects: int, g_channel_mode: str = "default"):
        super().__init__(root)
        ...
        # G channel encoding mode (Phase 2/3 ablation)
        # - 'default': 3-ch (R + G=obj_id/N + B=zero), 기존 cnn_train_compound default
        # - 'sparse':  3-ch, G 만 (R > 0) defect 영역에만 활성화 (Phase 2)
        # - 'onehot':  (1+N)-ch, R + N obj binary masks (Phase 3, ★ 정확도 1순위)
        self.g_channel_mode = str(g_channel_mode)
        # G channel resize: 항상 _chipgrid_resize.block_expand_2d 사용 (categorical 보존,
        # BICUBIC noise 회피). 정수 배수 (32→384=12x) 면 NEAREST 와 동일, 비정수면 균등 spread.
    ...
    def num_input_channels(self) -> int:
        """Return number of input channels for current mode."""
        if self.g_channel_mode in ("onehot", "adapter"):
            return 1 + self.n_chip_objects                                             # R + N obj masks
        return 3                                                                       # default / sparse

    def _load_3ch(self, png_path: str) -> torch.Tensor:
        """Load palette PNG + obj_id .npy → [C, H, W] float32 ∈ [0, 1].

        C depends on g_channel_mode:
            'default'/'sparse' → 3 (R + G + B)
            'onehot'           → 1 + n_chip_objects (R + N obj binary masks)
        """
        ...
        # Load raw obj_id (h, w) — flat basename lookup, subfolder-agnostic.
        basename = Path(png_path).stem
        obj_path = self._npy_map.get(basename)
        if obj_path is not None and obj_path.exists():
            obj_id_raw = np.load(obj_path).astype(np.uint8)                            # (h, w) ∈ {0..N}
        else:
            obj_id_raw = None
            if not self._missing_obj_id_warned:
                print(f"[FailObjImageFolder] missing obj_id for basename={basename!r} — G/onehot=zeros (warn once)", flush=True)
                self._missing_obj_id_warned = True

        # block_expand: chip-block categorical-preserving upscale (BICUBIC noise 회피)
        # 정수 배수 (e.g. 32→384 = 12x): 각 chip 정확히 base px (NEAREST 동일)
        # 정수 안 됨 (e.g. 32→200): 일부 chip base+1 px (균등 spread)
        from _chipgrid_resize import block_expand_2d

        # === Phase 3: ONE-HOT MULTI-CHANNEL (mode=onehot or adapter) ===
        # adapter mode 는 동일하게 (1+N)-ch 출력. 모델 단에서 stem adapter 로 N+1 → 3 projection.
        if self.g_channel_mode in ("onehot", "adapter"):
            channels = [r]
            for k in range(1, self.n_chip_objects + 1):
                if obj_id_raw is not None:
                    mask_k = (obj_id_raw == k).astype(np.uint8)                       # binary (0 or 1)
                    mask_expanded = block_expand_2d(mask_k, self.img_size)             # (img, img) categorical
                    ch_k = torch.from_numpy(mask_expanded.astype(np.float32)).unsqueeze(0)
                else:
                    ch_k = torch.zeros((1, self.img_size, self.img_size), dtype=torch.float32)
                channels.append(ch_k)
            return torch.cat(channels, dim=0)                                          # (1+N, H, W)

        # === default / sparse 3-ch path ===
        if obj_id_raw is not None:
            obj_expanded = block_expand_2d(obj_id_raw, self.img_size)                  # (img, img) uint8
            g = torch.from_numpy(obj_expanded.astype(np.float32) / float(self.n_chip_objects)).clamp_(0.0, 1.0).unsqueeze(0)
        else:
            g = torch.zeros((1, self.img_size, self.img_size), dtype=torch.float32)

        # Phase 2: sparse — R 의 defect 영역만 G 활성화
        if self.g_channel_mode == "sparse":
            defect_mask = (r > 0.0).float()
            g = g * defect_mask
    ...
    def _augment(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Rotation/affine apply to all channels — obj/G categorical fractional is
        intentional spatial cue. Works for any number of channels (3 or 1+N).
        """
        # ±15° rotation
        ...
        # Gaussian noise on R (channel 0) only — obj channels are categorical signals
    ...
    def _normalize(self, x: torch.Tensor) -> torch.Tensor:
        ...
        - 3-ch (default/sparse): ImageNet mean/std (legacy compat with R-only ckpt init).
        - (1+N)-ch (onehot): R uses ImageNet mean/std[0]; obj channels passed through (0~1 range).
        """
        if x.shape[0] == 3:
            return (x - self._IMAGENET_MEAN) / self._IMAGENET_STD
        # onehot: R 채널 만 ImageNet 정규화
        r_mean = self._IMAGENET_MEAN[0:1]                                              # (1, 1, 1)
        r_std = self._IMAGENET_STD[0:1]
        r_norm = (x[0:1] - r_mean) / r_std
        obj = x[1:]                                                                    # (N, H, W) raw 0~1
        return torch.cat([r_norm, obj], dim=0)
...
def _inflate_stem_weight(w_pretrained: torch.Tensor, target_in_chans: int) -> torch.Tensor:
    """Expand pretrained stem.0.weight (out, 3, k, k) → (out, target_in_chans, k, k).

    Phase 3 — for in_chans > 3 (one-hot multi-channel):
      - First channel: copy R channel of pretrained (idx 0)
      - Channels 1..(target-1): pretrained R channel weight / target_in_chans (Carreira & Zisserman 2017 inflation)

    For in_chans == 3, return pretrained as-is.
    For in_chans < 3, take channel 0 mean.
    """
    out, src_in, k1, k2 = w_pretrained.shape
    if target_in_chans == src_in:
        return w_pretrained
    new_w = torch.zeros((out, target_in_chans, k1, k2), dtype=w_pretrained.dtype)
    if target_in_chans > src_in:
        # R 채널은 pretrained R 그대로
        new_w[:, 0] = w_pretrained[:, 0]
        # obj-channels: R channel weight 의 1/(target-1) 균등 분배 (signal magnitude 보존)
        scale = 1.0 / max(target_in_chans - 1, 1)
        for c in range(1, target_in_chans):
            new_w[:, c] = w_pretrained[:, 0] * scale
    else:
        # target < src: 평균 collapse
        new_w[:, 0] = w_pretrained.mean(dim=1)
    return new_w


def build_model(num_classes: int, drop_path_rate: float = 0.0,
    ...
    in_chans: input channels. default 3 (R+G+B compound). Phase 3 (one-hot
    multi-channel) 시 1+N (e.g. 6).  pretrained stem.0.weight 를 inflation.
    """
    LOCAL_WEIGHTS_DIR.mkdir(parents=True, exist_ok=True)
    wf = _backbone_weights_path()

    def _load_with_inflation(full_sd: dict) -> nn.Module:
        ...
        for k, v in full_sd.items():
            if k not in m_sd:
                skipped += 1
                continue
            if k == "stem.0.weight" and v.shape != m_sd[k].shape:
                # Inflate pretrained 3-ch stem to in_chans
                v_inflated = _inflate_stem_weight(v, m_sd[k].shape[1])
                if v_inflated.shape == m_sd[k].shape:
                    compat[k] = v_inflated
                else:
                    skipped += 1
                continue
            if m_sd[k].shape != v.shape:
                skipped += 1
                continue
            compat[k] = v
        m.load_state_dict(compat, strict=False)
        return m, len(compat), skipped

    if wf.exists():
        full_sd = torch.load(wf, map_location="cpu")
        m, loaded, skipped = _load_with_inflation(full_sd)
        print(f"[backbone] loaded from local mirror: {wf.name} "
              f"(in_chans={in_chans}, {loaded} keys loaded, {skipped} skipped)",
              flush=True)
    else:
        ...
        m, loaded, skipped = _load_with_inflation(m_full.state_dict())
    return m


class AdapterModel(nn.Module):
    """Stem adapter: (1+N)-ch input → 1×1 Conv → 3-ch → ConvNeXtV2 backbone.

    backbone 의 in_chans=3 그대로 → FCMAE pretrained stem.0.weight (3채널) 100% 보존.
    추가 params = adapter Conv2d(in_chans, 3, 1) ≈ in_chans*3 + 3 (very small).

    Init 정책 (사용자 의도: 처음에 R 만 backbone 에 전달, obj_id 채널은 학습으로 weight 형성):
        - adapter weight[c=0..2, ch=0, 1, 1] = 1.0/3.0 (R 을 RGB 각 채널에 1/3 씩 투사 — grayscale-like)
          [실제로: each output channel uses R contribution = 1, others = 0]
          ImageNet RGB pretrained backbone 입장에서 grayscale 이미지 분포 와 비슷한 입력
        - adapter weight[c=0..2, ch=1..N, 1, 1] = 0  (obj_id 채널 처음엔 무시)
        - bias = 0
        → 학습 시 adapter Conv 가 각 obj_id 채널의 우선순위를 backward 로 결정
    """
    def __init__(self, total_input_channels: int, num_classes: int,
                 drop_path_rate: float = 0.0):
        super().__init__()
        self.total_input_channels = int(total_input_channels)
        self.adapter = nn.Conv2d(self.total_input_channels, 3, kernel_size=1, bias=True)
        # Init: R (channel 0) → all 3 RGB output channels (grayscale-like). obj channels → 0.
        with torch.no_grad():
            self.adapter.weight.zero_()
            for out_c in range(3):
                self.adapter.weight[out_c, 0, 0, 0] = 1.0
            self.adapter.bias.zero_()
        # Backbone with FCMAE 3ch pretrained
        self.backbone = build_model(num_classes=num_classes,
                                    drop_path_rate=drop_path_rate, in_chans=3)

    def forward(self, x):
        # x: (B, 1+N, H, W) — R + N obj binary masks (already block_expand resized)
...
def save_confusion_matrix_combined(val_res, test_res, out_path: Path):
    ...
    figsize / fontsize 는 class 수에 따라 자동 조절.
    """
    try:
        import matplotlib
        matplotlib.use("Agg")
...
def main():
    ...
    p.add_argument("--active-classes-yaml", default=None,
                   help="YAML with classes: list — 학습 시 그 class 만 사용. "
                        "default: 모든 class (data-dir 의 폴더명 기반, EXCLUDE_CLASSES 제외)")
    ...
    p.add_argument("--init-from", default=None,
                   help="TAPT init: wafer best `best_model.pth` 경로. backbone weight만 load. "
                        "같은 num_classes 면 head 도 reuse (Phase 1 권장: --init-from logs_wafer/overall/best_model.pth)")
    p.add_argument("--g-channel-mode", default="default",
                   choices=["default", "sparse", "onehot", "adapter"],
                   help="G 채널 encoding ablation. "
                        "default=3-ch (R+G=obj_id/N+B=zero, 기존), "
                        "sparse=3-ch (G 만 R>0 영역에서 활성화, Phase 2), "
                        "onehot=(1+N)-ch (R + N obj binary masks, ★ Phase 3), "
                        "adapter=(1+N)-ch with 1×1 Conv stem adapter (1+N → 3) → "
                        "ConvNeXtV2-base FCMAE pretrained 3ch backbone 그대로 사용.")
    ...
    # active classes (yaml) 적용 — FilteredImageFolder.find_classes 가 _ACTIVE_CLASSES 참조
    if args.active_classes_yaml:
        active_path = Path(args.active_classes_yaml)
        if not active_path.exists():
            raise FileNotFoundError(f"active classes YAML not found: {active_path}")
        with active_path.open(encoding="utf-8") as f:
            yc = yaml.safe_load(f) or {}
        active_list = list(yc.get("classes", []))
        if not active_list:
            raise ValueError(f"active classes YAML has no classes: {active_path}")
        data_classes = {
            d.name for d in Path(args.data_dir).iterdir()
            if d.is_dir() and d.name not in EXCLUDE_CLASSES
        }
        missing_active = [c for c in active_list if c not in data_classes]
        if missing_active and not args.allow_missing_active_classes:
            raise ValueError(
                f"active classes not found in {args.data_dir}: {missing_active}"
            )
        if missing_active:
            lg.info(f"[active-classes] missing classes dropped: {missing_active}")
        set_active_classes(active_list)
        lg.info(f"[active-classes] {len(active_list)} classes from {args.active_classes_yaml}")

    # 3-channel feature pipeline: R=palette_idx/31, G=obj_id/n_chip_objects, B=zero. block_expand resize.
    ...
    # ===== Model =====
    # Phase 3: in_chans depends on g_channel_mode (onehot → 1 + n_chip_objects)
    in_chans = full_eval.num_input_channels
    lg.info(f"[model] in_chans={in_chans} (mode={args.g_channel_mode})")
    if args.g_channel_mode == "adapter":
        # 1×1 Conv stem adapter (1+N → 3) → ConvNeXtV2 3ch FCMAE pretrained backbone
        model = AdapterModel(total_input_channels=in_chans, num_classes=num_classes,
                             drop_path_rate=args.stochastic_depth).to(device)
        n_adapter = sum(p.numel() for p in model.adapter.parameters())
        lg.info(f"[model] AdapterModel — adapter params={n_adapter} (Conv2d({in_chans},3,1)) "
                f"+ ConvNeXtV2-base 3ch FCMAE pretrained backbone")
    else:
        model = build_model(num_classes, drop_path_rate=args.stochastic_depth,
                            in_chans=in_chans).to(device)
    if args.init_from:
        # TAPT — wafer best 의 backbone weight 로 init.
        # ckpt 형태: {"model": state_dict, "classes": [...], "ema_state": {...}, ...}
        # Phase 1 개선: 같은 num_classes 면 head 도 reuse (compound 와 wafer 가 같은 33-class
        # wafer 분류 task 라 head 도 재학습 손해. compound 의 R 채널 의미가 R-only 의
        # RGB triplet 과 약간 다르나, stages weight (chip-grid pattern) 는 도메인 공유).
        ckpt = torch.load(args.init_from, map_location="cpu", weights_only=False)
        sd_full = ckpt.get("model", ckpt)
        init_classes = ckpt.get("classes")
        head_match = (init_classes is not None and len(init_classes) == num_classes)
        if head_match:
            load_sd = sd_full
        else:
            load_sd = {k: v for k, v in sd_full.items() if not k.startswith("head.")}
        # filter shape mismatch (e.g. different in_chans)
        m_sd = model.state_dict()
        compat = {k: v for k, v in load_sd.items()
        ...
        lg.info(f"  loaded {len(compat)} keys (head_reuse={head_match}, "
                f"init_classes={len(init_classes) if init_classes else '?'}, "
                f"compound_classes={num_classes})")
        lg.info(f"  head re-init={len(head_missing)} keys, "
    ...
    for ep in range(1, args.epochs + 1):
        ...
        if (smooth_f1 > best_score) and not guard_block:
            ...
            ckpt = {"epoch": ep, "model": model.state_dict(), "classes": classes,
                    ...
                    # Phase 2/3 metadata for inference reproducibility
                    "g_channel_mode": args.g_channel_mode,
