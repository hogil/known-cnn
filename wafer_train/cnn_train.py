...
# -*- coding: utf-8 -*-
"""
CNN classifier training (ConvNeXtV2 base FCMAE).
...
from _ddp import init_ddp, wrap_ddp, make_sampler, cleanup_ddp, unwrap, barrier
...
_ALLOW_MISSING_ACTIVE = False


def set_active_classes(classes_list, allow_missing: bool = False):
    """Restrict FilteredImageFolder to given class list (in addition to EXCLUDE_CLASSES)."""
    global _ACTIVE_CLASSES, _ALLOW_MISSING_ACTIVE
    _ACTIVE_CLASSES = list(classes_list) if classes_list else None
    _ALLOW_MISSING_ACTIVE = bool(allow_missing)
...
class FilteredImageFolder(ImageFolder):
    ...
    def find_classes(self, directory):
        ...
        if _ACTIVE_CLASSES is not None:
            missing = [c for c in _ACTIVE_CLASSES if c not in classes]
            if missing and not _ALLOW_MISSING_ACTIVE:
...
def main(default_data_dir: Optional[str] = None,
    ...
    p.add_argument("--active-classes-yaml", default=None,
                   help="YAML: classes:[name, ...] — restrict ImageFolder to given list")
    p.add_argument("--allow-missing-active-classes", action="store_true",
                   help="active YAML 안 class 가 data dir 에 없어도 skip (default: SystemExit)")
    ...
    # ===== DDP setup (env-driven, torchrun) =====
    ddp = init_ddp()
    is_main = ddp["is_main"]

    # ===== Resource guard (시작 점검, rank 0 만) =====
    if is_main:
        ...
        a = {"device": "cuda"}                                                              # other ranks trust rank 0
    ...
    if ddp["enabled"] and args.weighted_sampler:
        if is_main:
            print("[!] --weighted-sampler ignored under DDP (DistributedSampler used instead)", file=sys.stderr)
        args.weighted_sampler = False

    # seed (per-rank distinct seed for sampler; ranks share base seed for model init)
    random.seed(args.seed); np.random.seed(args.seed); torch.manual_seed(args.seed)
    torch.cuda.manual_seed_all(args.seed)
    torch.backends.cudnn.benchmark = True

    # device — DDP 사용 시 local_rank GPU, 아니면 guard 결과
    device = ddp["device"] if ddp["enabled"] else torch.device(a["device"])
    ...
    if args.active_classes_yaml:
        ...
        with open(args.active_classes_yaml, "r", encoding="utf-8") as f:
            _ay = _yaml.safe_load(f) or {}
        _active = _ay.get("classes", []) or []
        set_active_classes(_active, allow_missing=args.allow_missing_active_classes)
        lg.info(f"[active-classes] {len(_active)} class enforced from {args.active_classes_yaml}: {_active}")
    ...
    # ===== DDP: train sampler partitions; val sampler stays None (all ranks full val for identical metrics) =====
    if ddp["enabled"]:
    ...
    # test DataLoader with paths (for save_pred_samples) — DDP 시 rank 0 만 평가
    test_ld = None
    if has_test and is_main:
    ...
    # ===== Optim (param group decision before DDP wrap) =====
    ...
    for ep in range(1, args.epochs + 1):
        if monitor.should_abort():
            ...
            break
        # DDP: re-shuffle DistributedSampler each epoch
        if ddp["enabled"] and train_sampler is not None and hasattr(train_sampler, "set_epoch"):
        ...
        if (smooth_f1 > best_score) and not guard_block:
            ...
            # save best ckpt — DDP: rank 0 만 (state_dict unwrap)
            if is_main:
                ckpt = {"epoch": ep, "model": unwrap(model).state_dict(), "classes": classes,
    ...
    # stop monitor (all ranks)
    monitor.stop()

    # === DDP: ranks > 0 stop here. rank 0 does post-loop file IO / rename / overall. ===
    if not is_main:
        cleanup_ddp(ddp)
        return
