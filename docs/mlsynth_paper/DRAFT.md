...
> whose contribution order (broad-venue repositioning; theory DEMOTED 2026-07-17 as a
> numbered contribution but UPGRADED 2026-07-17 from support to PREDICTIVE: a compact
> "when does recovery happen?" subsection + Appendix A) is:
> (i) the **operator-match
...
> single->multi **setting** + the
> objective of **performance maximization** within it, with the oracle as a
> multi-label-trained upper reference; (iv) the **method** = FCM-PM + val-margin +
> naive-Bayes reject (the partition-domain instantiation), the winner on the chip
> partition domain and the best FAR-controlled content-blind operator on WM38; (v)
...
them. Our answer is a *characterization of the achievable frontier* of this
setting: we delineate what single-label supervision lets a content-blind
synthesizer recover, what it provably cannot, and why -- and turn the
recoverable part into a method that learns multi-label recognizers end to end,
with an annotation-free reliability certificate.
...
law**. This is the criterion for the *recoverable* half of that frontier, now a
*measured* principle — the
...
(chip-internal maps). We verify the crossover directly and in **both** directions, on real
data and under a controlled mechanism study. On the real chip domain, whose true law is a
partition, FCM-PM beats overlay decisively on both bit-F1 and FAR under an identical protocol
(Sec 5.2.1), because pixelwise-max mis-models a partition; and the same partition win
reproduces on a *real, public, non-constructed* benchmark — SVHN Street View House Numbers,
whose partition law we did not define — where the matched partition-placement operator
...
and recovering the oracle in every case (4/4; Sec 5.1.1) — so the operator-match partition win
is established on real, public data (SVHN, chip) and cleanly isolated across four constructed
datasets in both directions, general and not reliant on internal data. The operator-match
criterion is measured before any training via **label fidelity** (every
...
Taken together, our contributions *characterize the achievable frontier* of
content-blind multi-label synthesis -- what single-label supervision can recover (a
predictive operator-match criterion), what it provably cannot (an appearance-floor
impossibility), how to certify the result (an annotation-free reliability guarantee),
and that the operator choice is itself learnable -- rather than merely choosing between
two operators:
...
1. **The operator-match law (a general principle for label-faithful synthesis).** The
   ...
   *verified* law with a matched head-to-head on the real chip domain (a partition law,
   ...
   pixelwise-max mis-models the partition and under-detects the second defect. **The same
   partition win reproduces on a real, public, non-constructed benchmark:** on SVHN Street
   View House Numbers (format-1 full house numbers, a genuine partition law we did *not*
   define -- multi-hot over the 10 digit classes, positions never used as labels; N=7566 real
   test house numbers, single-digit pool 30,000 real crops, no leak) the matched
   partition-placement operator reaches eval bit-F1 0.8855 +-0.0051 (mAP 0.9442), beats
   overlay (0.5141), CutMix (0.6688), and MixUp (0.7638) with no std overlap, and recovers a
   ...
   *decomposes* into a closeable *geometry* component (recovered by matching real region shapes) and
   ...
   in every regime on all four datasets it beats not only the mismatched operator but also
   CutMix and MixUp, with no std overlap (4/4; e.g. MNIST partition-placement 0.9014 +-0.014
   vs overlay 0.6568 +-0.033, CutMix 0.8660 +-0.002, MixUp 0.6142 +-0.050; superposition
   overlay 0.7650 +-0.004 vs partition-placement 0.4295 +-0.027, CutMix 0.4463 +-0.023, MixUp
   0.4744 +-0.043) and recovers the oracle in each direction (e.g. MNIST partition 0.9014 vs
   0.9072; superposition 0.7650 vs 0.7650), the matched-minus-mismatched gap 0.18-0.34
   throughout (matched clears even the strongest content-blind baseline by 0.035-0.29,
   tightest on MNIST partition where matched 0.9014 still clears CutMix 0.8660) with absolute
   bit-F1 declining on harder sources while the flip direction stays invariant. So the
   operator-match partition win/ordering is now verified on **three** real-public/real domains
   of different type -- real digits (SVHN, with full oracle-recovery), mainstream remote sensing
   (land cover, ordering-only with an honest sim-to-real no-recovery limit), and the real fab
   chip domain (private) -- with the four constructed datasets supplying the clean
   both-directions mechanism control. This makes operator-match a general, publicly verifiable
   law established on real, public, non-constructed data (not a dataset-specific artifact),
   while we explicitly own that land cover extends the *ordering*-generality without extending
...
2. **An annotation-free FAR guarantee (a practical reliability layer).** A strict source-only
   ...
   operator-agnostic: it holds for *any* operator, so it is a transferable practical benefit
   rather than a differentiator of our method, and it is absent from prior operator-only work
   (e.g. Shin et al. 2022). A full multi-alpha calibration curve shows the realized FAR
   tracking the target across alpha in [0.5%, 10%] to within 0.153 pp for every operator.
3. **The single->multi setting and performance maximization within it.** We train
...
4. **The method (partition-domain instantiation): FCM-PM + val-margin + naive-Bayes
...
Our work draws on four literatures -- combination-style data augmentation, weak
multi-label supervision, single-to-mixed defect synthesis, and reliability (selective
and conformal prediction, and label noise) -- but reduces to none of them. Each supplies
one ingredient; our contribution is the framework that **unifies** operator choice
(which content-blind synthesizer to trust, decided before training), reliability (an
annotation-free false-alarm guarantee), and a recoverability characterization (what
single-label data can and cannot recover) -- none of which any single family provides.
We review each below and mark the distinction.
...
- **Synthetic data and sim-to-real.** The single-to-mixed methods above are all
  sim-to-real in disguise -- they train on synthesized combinations and test on real
  ones -- so their reliability hinges on how faithfully synthesis reproduces real
  co-occurrence appearance. We make this sim-to-real gap explicit on remote-sensing land
  cover (Sec 5.1.3), decomposing it into a closeable *geometry* component (matched real
  region shape) and an irreducible *appearance* component -- the same appearance floor
  our theory (Sec Theory) shows single-label marginals cannot cross -- which is what
  separates the domains where blind synthesis recovers the oracle from those where it
  can only close part of the gap.
- **Selective prediction, conformal calibration, and label noise.** Our reliability
  layer builds on selective prediction (abstaining below a confidence threshold to trade
  coverage for reliability) and makes its false-alarm control *distribution-free* via
  split-conformal calibration (Lei et al., 2018), turning a finite known-good set into a
  finite-sample marginal FAR guarantee under exchangeability, with *no* real-normal
  training label. Separately, our label-fidelity-to-risk argument is an instance of
  learning under class-conditional label noise (Natarajan et al., 2013): an
  evidence-destroying operator (CutMix at high patch fraction) manufactures label noise
  *at synthesis time*, so its downstream risk follows the same class-conditional-noise
  mechanics. Neither line, however, decides *which* synthesizer to trust in the first
  place; our operator-match criterion supplies that, upstream of both.
...
#### 5.1.1 Controlled mechanism isolation: the partition/superposition flip on four public sources

The chip head-to-head (Sec 5.2.1) and the SVHN house-number benchmark (Sec 5.1.2) establish
the partition win on **real** data; this experiment complements them with a **controlled
mechanism isolation** that cleanly separates the combination-law variable — same sources,
only the combination law changed, in **both** directions — on **author-constructed**
tiled-digit canvases built from four public sources (MNIST, FashionMNIST, KMNIST, and
EMNIST-letters, 26 classes) under one identical protocol. For each source two multi-label
domains are **constructed** from the **same** public glyphs on a 56x56 two-by-two-cell canvas,
trained with the **same** shared SmallCNN (20 epochs, 3 seeds), changing **only** the domain's
true two-glyph combination law: a **partition** domain (the two glyphs occupy disjoint cells)
and a **superposition** domain (the two glyphs share one cell, combined by pixelwise max). In
each domain the two content-blind operators run head-to-head — partition-placement (the
FCM-PM analogue) and overlay/max-union — alongside two further content-blind baselines
(CutMix and MixUp), the single-only floor, and an oracle trained on true-law combos (upper
reference). Eval = held-out multi-label test set constructed from that source's own test
split; matched = the operator whose combine matches the domain's true law. Because the
canvases are constructed, this study is a clean mechanism control, not a real-data benchmark
— that role is played by SVHN and chip.

| dataset        | regime        | matched (mean +-std) | mismatched (mean +-std) | cutmix (mean +-std) | mixup (mean +-std) | single | oracle |
|----------------|---------------|----------------------|-------------------------|---------------------|--------------------|--------|--------|
| MNIST          | partition     | **0.9014 +-0.014**   | 0.6568 +-0.033          | 0.8660 +-0.002      | 0.6142 +-0.050     | 0.4559 | 0.9072 |
| MNIST          | superposition | **0.7650 +-0.004**   | 0.4295 +-0.027          | 0.4463 +-0.023      | 0.4744 +-0.043     | 0.3685 | 0.7650 |
| FashionMNIST   | partition     | **0.7136 +-0.011**   | 0.5367 +-0.045          | 0.6720 +-0.011      | 0.6604 +-0.034     | 0.3000 | 0.7106 |
| FashionMNIST   | superposition | **0.5841 +-0.024**   | 0.3099 +-0.016          | 0.3461 +-0.004      | 0.4222 +-0.031     | 0.2483 | 0.5843 |
| KMNIST         | partition     | **0.6811 +-0.013**   | 0.4727 +-0.045          | 0.6267 +-0.006      | 0.6441 +-0.017     | 0.3617 | 0.6759 |
| KMNIST         | superposition | **0.5549 +-0.011**   | 0.3345 +-0.008          | 0.3780 +-0.014      | 0.4367 +-0.003     | 0.2540 | 0.5566 |
| EMNIST-letters | partition     | **0.6146 +-0.011**   | 0.3465 +-0.030          | 0.5493 +-0.007      | 0.5057 +-0.035     | 0.3188 | 0.6119 |
| EMNIST-letters | superposition | **0.4045 +-0.017**   | 0.2284 +-0.013          | 0.2464 +-0.016      | 0.3086 +-0.027     | 0.2039 | 0.4027 |

Matched = the blind operator whose join reproduces the regime's true law (partition:
partition-placement; superposition: overlay/max-union); mismatched = the other; cutmix and
mixup are the further content-blind baselines; single = single-only floor; oracle = trained on
true-law combos (upper reference). In every row matched beats all three content-blind
baselines (mismatched, cutmix, mixup) with no std overlap and recovers the oracle.

**The flip holds 4/4, in both directions, and the matched operator dominates every
content-blind baseline with no std overlap.** In every dataset and regime the **matched**
blind operator wins and the mismatched one is clearly worse, the winner and loser trading
places exactly as the law changes; across all eight rows the matched-minus-mismatched bit_F1
gap is 0.18-0.34 and no matched/mismatched std bands overlap. On MNIST the partition domain
gives partition-placement 0.9014 +-0.014 vs overlay 0.6568 +-0.033 (+0.245), and the
superposition domain gives overlay 0.7650 +-0.004 vs partition-placement 0.4295 +-0.027
(+0.335); the same clean flip recurs on FashionMNIST (0.7136/0.5367 partition, 0.5841/0.3099
superposition), KMNIST (0.6811/0.4727; 0.5549/0.3345), and EMNIST-letters (0.6146/0.3465;
0.4045/0.2284). **The matched operator is moreover the best content-blind choice, not merely
better than the mismatched one:** in every regime on all four datasets it also beats CutMix
and MixUp with no std overlap, clearing the strongest content-blind baseline in each row by
0.035-0.29 (tightest on MNIST partition, where matched 0.9014 still clears CutMix 0.8660:
0.887 > 0.868); no content-blind baseline ever ties or beats matched. **The matched operator
recovers the oracle within noise in every dataset and both directions** (e.g. MNIST partition
0.9014 vs oracle 0.9072, superposition 0.7650 vs oracle 0.7650; FashionMNIST 0.7136 vs
0.7106, 0.5841 vs 0.5843; KMNIST 0.6811 vs 0.6759, 0.5549 vs 0.5566; EMNIST 0.6146 vs 0.6119,
0.4045 vs 0.4027), so matched blind synthesis is statistically indistinguishable from training on the true law,
...
direction across four sources under one controlled protocol. **Framing (honest scope):**
because the tiled canvases are **author-constructed**, this study is the clean **mechanism
control** — it isolates the combination-law variable (identical sources, only the law changes)
in both directions across four datasets — and it is **not** itself a real-data benchmark; the
real-data partition evidence is the SVHN house-number benchmark (Sec 5.1.2, real public, a
partition law we did not define) and the chip head-to-head (Sec 5.2.1, real fab). Together the
evidence spans a controlled mechanism (four constructed tiled-digit datasets, both
directions), a real **public** partition benchmark (SVHN), and a real chip partition domain —
...
The controlled four-dataset study above isolates the combination-law mechanism on
author-constructed tiled-digit canvases; here we verify the partition-direction win on a
**real, public, non-constructed** benchmark whose partition law we did **not** define — SVHN
Street View House Numbers. We use the format-1 full house-number images, which are genuinely
partition-structured: the digits sit side by side in disjoint image regions, a partition
combination law fixed by the photographed scene rather than by us. The multi-hot target is
over the 10 digit classes (*which* digits appear); digit *positions* are never used as
labels, keeping the setting content-blind. The single-digit training pool is 30,000 real SVHN
crops and the evaluation set is N=7566 *real* multi-digit house numbers, restricted to two
distinct digits; there is no leak — the pool and the oracle are drawn from SVHN-*train*,
evaluation from SVHN-*test*. We train a ResNet-18 from scratch (grayscale, 32x64 canvas, 3
seeds) and run partition-placement (the FCM-PM analogue, matched to the house-number partition
law), overlay/max-union (mismatched), CutMix, MixUp, the single-only floor, and an oracle
trained on *real* multi-digit house numbers (upper reference):

| arm (SVHN, 3 seeds)                     | eval bit_F1 (mean +-std) | mAP    |
|-----------------------------------------|--------------------------|--------|
| partition-placement (**matched, ours**) | **0.8855 +-0.0051**      | 0.9442 |
| overlay / max-union (mismatched)        | 0.5141 +-0.0531          | 0.7837 |
| cutmix                                  | 0.6688 +-0.0399          | 0.8451 |
| mixup                                   | 0.7638 +-0.0071          | 0.8744 |
| single_only (floor)                     | 0.5326 +-0.0242          | 0.8769 |
| oracle (real multi-digit training)      | 0.8947 +-0.0156          | 0.9546 |

**On real photographed partition data the matched operator wins decisively and recovers the
oracle.** Partition-placement reaches eval bit_F1 0.8855 +-0.0051 (mAP 0.9442), beating the
mismatched overlay (0.5141 +-0.0531, +0.371) and **both** other content-blind baselines —
CutMix (0.6688 +-0.0399) and MixUp (0.7638 +-0.0071) — with **no** mean+-std overlap against
any of the three, and it sits well above the single-only floor (0.5326 +-0.0242). Crucially it
recovers the oracle within noise: 0.8855 vs the real-multi-digit oracle's 0.8947 +-0.0156 (~99%,
overlapping std bands; mAP 0.9442 vs 0.9546). The oracle did **not** collapse — at 0.8947 / mAP
0.9546 the real photographed task is clearly learnable — so the comparison is *informative*, not
an undertrained tie: matched blind synthesis reaches oracle-level performance on a task the
oracle solves well. This is the operator-match partition win on a **real, public** benchmark —
the public counterpart to the internal chip head-to-head (Sec 5.2.1) and the direct answer to
whether the win survives outside author-constructed data: it does, on genuine partition structure
we did not impose. Runner: `multilabel_synth/run_operator_match_svhn.py` (committed).

**Scope (honest).** SVHN here is a deliberately *clean* real-public partition instance, **not**
a mainstream-scale multi-label benchmark: the task is restricted to two distinct digits, and
digit *positions* are discarded so the target is multi-hot *presence* only -- a self-imposed
content-blind framing that is natural in the fab but artificial on house numbers, where position
is the usual supervised signal. One per-seed wrinkle (CutMix seed 0 over-alarms with normal-FAR
1.0 on the blank-canvas arm) does **not** touch the bit-F1 verdict, which holds across all three
...
(0.99-1.00) with well-separated negatives (eval neg prob 0.1266; NI-FAR 0.00%).

**Fairness of the head-to-head (limitation, addressed).** Both arms share *one* recipe
(T7/LS0.295/g3/grid9) and the *same* naive-Bayes (I10) acceptor, and that recipe/acceptor was
tuned on the FCM-PM champion, so a rigorous reader may ask whether overlay's higher FAR is
partly an artifact of running it in FCM-PM's neighborhood. This is a deliberate single-variable
design -- only the multi-defect synthesis operator changes, every other factor held fixed -- and
the result is not a tuning artifact: overlay's failure is **structural**, not a mis-set
threshold. Its combination bit_F1 collapses to 0.61-0.80 because pixelwise-max under-detects the
second defect (an error no acceptor threshold can recover), **and** it simultaneously over-fires
on negatives (NI-FAR 19.00%); a single shared acceptor cannot both raise a suppressed positive
and suppress an over-fired negative, so retuning it toward overlay trades one failure for the
other rather than closing the gap. Decisively, the *same* partition win reproduces where overlay
is under *no* shared FCM-PM recipe: on real, public SVHN with per-arm ResNet-18 trained from
scratch (Sec 5.1.2) and across the four constructed datasets, each under its *own* protocol
(Sec 5.1.1). The shared-recipe caveat therefore bounds only the *magnitude* of the chip gap, not
its *direction*.
...
The *same partition win reproduces on a real, public, non-constructed benchmark* — SVHN
Street View House Numbers (format-1 house numbers, a partition law we did not define), where
partition-placement reaches eval bit_F1 0.8855 +-0.0051, beats overlay (0.5141), CutMix
(0.6688), and MixUp (0.7638) with no std overlap, and recovers a non-collapsed
real-multi-digit oracle (0.8947, ~99%; Sec 5.1.2) — and the same flip is isolated in **both**
directions under a controlled mechanism study on four constructed public-source datasets
(MNIST, FashionMNIST, KMNIST, EMNIST-letters; Sec 5.1.1), where the matched operator recovers
the oracle 4/4. So the operator-match partition win is measured on the real chip partition
domain **and** a real public partition benchmark (SVHN), with the four-dataset study supplying
the clean both-directions mechanism control — a general law, publicly verifiable and not
reliant on internal data.
...
**The guarantee is a practical reliability layer; it holds for any operator.** The
split-conformal FAR guarantee — a finite-sample, distribution-free bound from
known-good normals — is textbook split-conformal that operator-only prior work (incl.
Shin22) omits, and it applies to *any* operator — every content-blind operator (including
our deployed FCM-PM, whose measured FAR the conformal layer turns into a certified guarantee)
and the overlay/summation join alike. It is therefore a **practical, transferable** reliability
benefit, **not** a differentiator of our method: it favors no particular operator, and on this
benchmark the overlay/summation join in fact gets the *highest* coverage (below).
...
therefore claim only that (i) the conformal *guarantee* is operator-agnostic and
annotation-free -- a practical, transferable reliability layer (Shin 2022 provides none), not
a differentiator of our operator -- and nearly free on our deployed
...
(Sec Theory) is one-directional and we do not use its density-mismatch lower bound to
prefer any operator; its geometry/appearance decomposition instead quarantines this density
gap to the support term, where it *predicts* nothing about recovery (the recovery trichotomy
is governed by the appearance term, ~0 for WM38 at pair level). The surviving contributions are the single→multi setting and
...
inspection maps. The same partition win reproduces on a **real, public, non-constructed**
...
All experiments use public datasets or a synthesizable source, fixed seeds, and released
runner scripts. The controlled four-source mechanism study runs through
`multilabel_synth/run_operator_match*.py` (MNIST, FashionMNIST, KMNIST, EMNIST-letters),
and its end-to-end learnable-gate variant through
`multilabel_synth/run_operator_match_learned.py`; the real-public partition benchmarks
run through `multilabel_synth/run_operator_match_svhn.py` (SVHN format-1 house numbers)
and `multilabel_synth/run_operator_match_landcover.py` (UC-Merced/DLRSD land cover), with
the few-shot real-tile sweep in `multilabel_synth/run_fewshot_ksweep_landcover.py`; the
wafer and chip results use the MixedWM38/chip synthesis-and-training pipeline. All
evaluation datasets are public -- MNIST, FashionMNIST, KMNIST, EMNIST, SVHN,
UC-Merced/DLRSD, and MixedWM38 -- and the chip-internal maps are synthetic and releasable
(the real fab-chip maps are internal, but the chip *synthesis* recipe -- FCM-PM,
complement grid, Pair-Mask, synthetic normals -- is fully specified in Sec 4). Seeds are
fixed and reported: the chip head-to-head uses three seeds (s99/s7/s42), and multi-seed
means and standard deviations accompany every headline comparison. Metrics (bit-F1 at
threshold 0.5, NORMAL-FAR, mAP, exact-match) and the split-conformal calibration
procedure are defined in Sec 3 and the reliability results of Sec 5.
