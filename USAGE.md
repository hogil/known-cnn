...
## 8. Multi-label Ablation (Stage 4-8)

8-stage paper-style ablation: single-label trained 모델을 multi-label 추론으로 확장. 자세한 이론·논문·설계는 `docs/multi-label/` 참조.

**Quickstart**:

```bash
# Stage 3 — multi-label evaluation 데이터 합성 (1.5h)
python _sample_gen_multi.py --n 2000 --workers 4 --seed 42
python _verify_multi.py --root D:/project/data/wm-811k/unknown_multi

# Stage 5a — calibration 분석 (학습 X, 즉시 가능)
python _calibration_analysis.py --input D:/project/data/wm-811k/unknown_multi \
   --positions-root D:/project/data/positions/unknown_multi \
   --plots-dir plots/calibration --results-dir results

# Stage 5b — D1-D8 threshold strategy sweep
python _threshold_sweep.py --val-logits results/val_logits.npy \
   --val-y results/val_y_multihot.npy \
   --classes-json results/val_classes.json \
   --strategies D1,D2,D3,D4,D5,D6,D7,D8

# Stage 4 Phase A — sigmoid heuristic 평가 (학습 X)
python _eval_multi_label.py --val-logits results/val_logits.npy \
   --val-y results/val_y_multihot.npy --classes-json results/val_classes.json

# Stage 6 — chip-wafer matching 7 mix combination
python _eval_chip_matching.py --combinations C1,C2,C3,C4,C5,C6,C7

# Stage 4 Phase B — AdaGC 학습 (option, ~30분/run)
python cnn_train_multilabel.py --loss adagc_asl --epochs 30 --batch 8 \
   --data-dir D:/project/data/wm-811k/unknown_multi \
   --classes-from logs_wafer/overall/best_model.pth --model-tag M4_adagc_asl

# Stage 4 Phase C — BCE/ASL 학습 (option, ~60분/run)
python cnn_train_multilabel.py --loss asl --epochs 30 --batch 8 \
   --gamma-pos 1 --gamma-neg 4 --clip 0.05 --label-smoothing 0.05 \
   --data-dir D:/project/data/wm-811k/unknown_multi --model-tag M3_asl_eff_ls

# Stage 8 — master report 종합
python _generate_master_report.py --results-dir results --plots-dir plots
```

**Loss 옵션** (multi_label_losses.py 의 build_loss factory):
- `bce` (M1) — BCEWithLogits + label_smoothing
- `asl` (M2) — Asymmetric Loss (Ridnik 2021) ★ 학계 SOTA
- `m3` — ASL + effective(0.9999) + ls=0.05
- `adagc_asl` (M4) — AdaGC + 0.5 × ASL hybrid (★ best 가설)
- `bce_warmup_asl` (M5) — BCE warmup → ASL fine-tune
- `focal_asl` (M6) — Focal positive + ASL negative
- `adagc_ls` (M7) — AdaGC + label_smoothing

**Threshold 옵션** (_threshold_sweep.py):
- D1 default 0.5
- D2 per-class F1 sweep
- D3 + Temperature scaling
- D4 + Platt scaling
- D5 Temp + Platt mix
- D6 IDF threshold
- D7 IDF + Temp + top-K floor
- D8 KNN_local + 모든 mix (★ best 가설)

**Matching 옵션** (_eval_chip_matching.py):
- C1 single heatmap_smooth
- C2 hybrid + consistency
- C3 3-method weighted ensemble
- C4 + distance-aware CRF
- C5 + percentile outlier
- C6 geometric mean ensemble
- C7 4-method geo + CRF + percentile (★ best 가설)

상세는 `docs/multi-label/` (이론) + `.claude/skills/multi-label-ablation/SKILL.md` (실행 패턴) + plan `~/.claude/plans/1-input-batch-hidden-patterson.md`.
