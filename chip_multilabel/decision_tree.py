...
Pipeline per chip:
    chip image -> invalid heuristic check
        ...
        -> False: 4-prob + threshold dict -> active set
                  ...
                  |active|>=3 : ★ user directive 260506 — NO truncate. raw active set
                                kept as class_key '<a>+<b>+<c>...' (NOT in COMBO_KEYS).
                                decision_type='3plus_active'. Auto-wrong against any GT,
                                quantifies how often model declares >2 simultaneous defects.
"""
...
class DecisionResult:
    class_key: str
    decision_type: str  # 'invalid' | 'normal' | 'single' | 'combo' | 'combo_collapsed' | '3plus_active'
...
def decide(
    ...
    if len(active) >= 4:
        # 4-active = no 4-combo class defined → auto-wrong (over-firing diagnostic)
        sorted_active = sorted(active)
        combo_key = "+".join(sorted_active)
        return DecisionResult(combo_key, "3plus_active", sorted_active, invalid_score)

    # |active| in {2, 3} — both supported as combo (260506: 3-combo added)
    sorted_active = sorted(active)
    combo_key = "+".join(sorted_active)
    if combo_key in COMBO_KEYS:
        return DecisionResult(combo_key, "combo", sorted_active, invalid_score)
    # Defensive collapse for unknown combo — pick top-1 prob.
