...
- I7  sigmoid with separate single / combo-additional thresholds (joint coord descent)
...
- I11 I7 + bank+scratch_rot pair-aware rescue (analyst, arXiv:2404.16193)
- I12 I7 + scratch+scratch_rot pair-aware rescue (260506, mirror of I11 for sc+sr weakness)
- I13 I7 + max-prob low-confidence Normal gate (260506, real-env Normal 80% lever)
...
# I11 — bank+scratch_rot pair-aware rescue (analyst's recommendation)
# Empirical: in bb+sr GT chips with prob_bb < 0.54, mean prob_sr = 0.591 (above 0.46).
# Rescue: if prob_bb in [I11_PAIR_LOW, T_bb) AND prob_sr in [I11_PAIR_LOW, T_sr) AND
# max(prob_fork, prob_scratch) < I11_OTHER_MAX -> declare bb+sr both active.
I11_PAIR_LOW = 0.40
I11_OTHER_MAX = 0.45

# I12 — scratch+scratch_rot pair-aware rescue (260506, mirror of I11 for sc+sr weakness)
# T9d sc+sr F1 = 0.755 (R=0.606): 39% sc+sr GT chips collapse to single sc/sr.
# Rescue: if prob_scratch in [I12_PAIR_LOW, T_sc) AND prob_scratch_rot in [I12_PAIR_LOW, T_sr) AND
# max(prob_bank_boundary, prob_fork) < I12_OTHER_MAX -> declare sc+sr both active.
I12_PAIR_LOW = 0.40
I12_OTHER_MAX = 0.45

# I13 — max-prob low-confidence Normal gate (260506, real-env Normal 80% lever)
# Independent from I10 entropy gate. If max(sigmoid prob over 4 train classes) < I13_NORMAL_PROB_MAX,
# declare Normal — catches "model unsure about everything" cases that entropy may miss.
I13_NORMAL_PROB_MAX = 0.55
...
def evaluate_cell(
    ...
    use_pair_rescue_bb_sr = False
    use_pair_rescue_sc_sr = False  # 260506 I12
    use_max_prob_normal = False    # 260506 I13
    if variant_id == "I0":
        ...
        threshold_dict = thr_val
        probs_eval = probs_full
        use_pair_rescue_bb_sr = True
    elif variant_id == "I12":
        # 260506 — sc+sr pair-aware rescue (mirror of I11)
        probs_full = sigmoid(L_train)
        thr_val = joint_macro_f1_threshold(probs_full[val_idx], y_val_multihot, list(TRAIN_CLASSES))
        threshold_dict = thr_val
        probs_eval = probs_full
        use_pair_rescue_sc_sr = True
    elif variant_id == "I13":
        # 260506 — max-prob low-confidence Normal gate (real-env Normal 80% lever)
        probs_full = sigmoid(L_train)
        thr_val = joint_macro_f1_threshold(probs_full[val_idx], y_val_multihot, list(TRAIN_CLASSES))
        threshold_dict = thr_val
        probs_eval = probs_full
        use_max_prob_normal = True
    elif variant_id == "I9":
    ...
    for local_i, gi in enumerate(eval_idx):
        ...
        if use_entropy_normal and not is_inv:
            ...
            if ent >= I10_ENTROPY_NORMAL_FRAC * log_C:
                ...
                continue
        # I13 — max-prob low-confidence Normal gate (260506)
        # If model is unsure about all 4 classes (max sigmoid prob < threshold), declare Normal.
        # Independent from I10 entropy gate — catches "uniformly low confidence" cases.
        if use_max_prob_normal and not is_inv:
            max_p = float(chip_probs.max())
            if max_p < I13_NORMAL_PROB_MAX:
                from .decision_tree import DecisionResult as _DR
                decision = _DR("Normal", "normal_max_prob", [], inv_score)
                preds_rows.append({
                    "cell_id": cell_id,
                    "chip_path": rec.chip_path,
                    "class_key": rec.class_key,
                ...
                if rec.class_key != "Normal":
                    error_rows.append({
                        ...
                        "true_labels": str(sorted(rec.true_active)),
                        "pred_labels": str([]),
                        "error_type": "wrong_normal_max_prob",
                        "per_class_probs": str({c: round(float(chip_probs[i]), 4) for i, c in enumerate(TRAIN_CLASSES)}),
                        "decision_type": "normal_max_prob",
                    })
                continue
        if use_pair_rescue_sc_sr and not is_inv:
            # I12 — scratch+scratch_rot pair-aware rescue (260506, mirror of I11)
            i_sc = TRAIN_CLASSES.index("scratch")
            i_sr = TRAIN_CLASSES.index("scratch_rot")
            i_bb = TRAIN_CLASSES.index("bank_boundary")
            i_fork = TRAIN_CLASSES.index("fork")
            p_sc = float(chip_probs[i_sc])
            p_sr = float(chip_probs[i_sr])
            p_other_max = max(float(chip_probs[i_bb]), float(chip_probs[i_fork]))
            if (p_sc >= I12_PAIR_LOW and p_sr >= I12_PAIR_LOW
                    and p_other_max < I12_OTHER_MAX):
                from .decision_tree import DecisionResult as _DR
                decision = _DR("scratch+scratch_rot", "pair_rescued_sc_sr",
                               ["scratch", "scratch_rot"], inv_score)
                preds_rows.append({
                    "cell_id": cell_id,
                    "chip_path": rec.chip_path,
                    "class_key": rec.class_key,
                    "true_labels": str(sorted(rec.true_active) or (["Normal"] if rec.is_normal_gt else (["Invalid"] if rec.is_invalid_gt else []))),
                    "pred_labels": str(["scratch", "scratch_rot"]),
                    "pred_class_key": "scratch+scratch_rot",
                    "prob_bank_boundary": float(chip_probs[0]),
                    "prob_fork": float(chip_probs[1]),
                    "prob_scratch": float(chip_probs[2]),
                    "prob_scratch_rot": float(chip_probs[3]),
                    "max_prob": float(chip_probs.max()),
                    "invalid_score": inv_score,
                    "decision_type": "pair_rescued_sc_sr",
                    "correct_multihot": bool((y_true_multihot_eval[local_i] == y_pred_multihot_eval[local_i]).all()),
                    "correct_11class": rec.class_key == "scratch+scratch_rot",
                })
                y_pred_multihot_eval[local_i, i_sc] = 1
                y_pred_multihot_eval[local_i, i_sr] = 1
                y_true_keys.append(rec.class_key)
                y_pred_keys.append("scratch+scratch_rot")
                if rec.class_key != "scratch+scratch_rot":
                    error_rows.append({
                        "cell_id": cell_id,
                        "chip_path": rec.chip_path,
                        "true_class_key": rec.class_key,
                        ...
                        "true_labels": str(sorted(rec.true_active)),
                        "pred_labels": str(["scratch", "scratch_rot"]),
                        "error_type": "wrong_pair_rescue_sc_sr",
                        "per_class_probs": str({c: round(float(chip_probs[i]), 4) for i, c in enumerate(TRAIN_CLASSES)}),
                        "decision_type": "pair_rescued_sc_sr",
                    })
                continue
        if use_pair_rescue_bb_sr and not is_inv:
            i_bb = TRAIN_CLASSES.index("bank_boundary")
            i_sr = TRAIN_CLASSES.index("scratch_rot")
            i_fork = TRAIN_CLASSES.index("fork")
            i_scratch = TRAIN_CLASSES.index("scratch")
            p_bb = float(chip_probs[i_bb])
            p_sr = float(chip_probs[i_sr])
            p_other_max = max(float(chip_probs[i_fork]), float(chip_probs[i_scratch]))
            # Trigger pair rescue regardless of individual thresholds — analyst intent
            if (p_bb >= I11_PAIR_LOW and p_sr >= I11_PAIR_LOW
                    and p_other_max < I11_OTHER_MAX):
                from .decision_tree import DecisionResult as _DR
                decision = _DR("bank_boundary+scratch_rot", "pair_rescued",
                               ["bank_boundary", "scratch_rot"], inv_score)
                preds_rows.append({
                    "cell_id": cell_id,
                    "chip_path": rec.chip_path,
                    "class_key": rec.class_key,
                    "true_labels": str(sorted(rec.true_active) or (["Normal"] if rec.is_normal_gt else (["Invalid"] if rec.is_invalid_gt else []))),
                    "pred_labels": str(["bank_boundary", "scratch_rot"]),
                    "pred_class_key": "bank_boundary+scratch_rot",
                    "prob_bank_boundary": float(chip_probs[0]),
                    "prob_fork": float(chip_probs[1]),
                    "prob_scratch": float(chip_probs[2]),
                    "prob_scratch_rot": float(chip_probs[3]),
                    "max_prob": float(chip_probs.max()),
                    "invalid_score": inv_score,
                    "decision_type": "pair_rescued",
                    "correct_multihot": bool((y_true_multihot_eval[local_i] == y_pred_multihot_eval[local_i]).all()),
                    "correct_11class": rec.class_key == "bank_boundary+scratch_rot",
                })
                y_pred_multihot_eval[local_i, i_bb] = 1
                y_pred_multihot_eval[local_i, i_sr] = 1
                y_true_keys.append(rec.class_key)
                y_pred_keys.append("bank_boundary+scratch_rot")
                if rec.class_key != "bank_boundary+scratch_rot":
                    error_rows.append({
                        "cell_id": cell_id,
                        "chip_path": rec.chip_path,
                        "true_class_key": rec.class_key,
