...
class CompoundWaferDataset(Dataset):
    ...
    def __init__(self, paths: List[str], img_size: int, n_chip_objects: int,
                 ...
                 g_channel_mode: str = "default"):
        self.paths = paths
        self.img_size = int(img_size)
        self.n_chip_objects = int(n_chip_objects)
        self.npy_map = npy_map
        self.labels = labels
        # Phase 2/3: G channel encoding mode (auto-detected from ckpt; see main())
    ...
    def _load_3ch(self, png_path: str) -> torch.Tensor:
        ...
        if obj_path is not None and obj_path.exists():
            obj_raw = np.load(obj_path).astype(np.uint8)
        elif not self._missing_warned:
            print(f"[predict] missing obj_id for basename={basename!r} — obj=zeros (warn once)",
        ...
        # Phase 3: one-hot multi-channel
        if self.g_channel_mode == "onehot":
            channels = [r]
            for k in range(1, self.n_chip_objects + 1):
                if obj_raw is not None:
                    mask = (obj_raw == k).astype(np.uint8)
                    mask_expanded = block_expand_2d(mask, self.img_size)
                    ch = torch.from_numpy(mask_expanded.astype(np.float32)).unsqueeze(0)
                else:
                    ch = torch.zeros((1, self.img_size, self.img_size), dtype=torch.float32)
                channels.append(ch)
            x = torch.cat(channels, dim=0)                                              # (1+N, H, W)
            # Phase 3 normalization: R only ImageNet, obj channels raw
            r_norm = (x[0:1] - IMAGENET_MEAN[0:1]) / IMAGENET_STD[0:1]
            return torch.cat([r_norm, x[1:]], dim=0)

        # Default / sparse 3-ch path
        if obj_raw is not None:
            obj_expanded = block_expand_2d(obj_raw, self.img_size)
            g = torch.from_numpy(obj_expanded.astype(np.float32) / float(self.n_chip_objects)) \
                      .clamp_(0.0, 1.0).unsqueeze(0)
        else:
