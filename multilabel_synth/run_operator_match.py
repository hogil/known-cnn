...
FIELDS = ["regime", "arm", "matched", "seed", "bit_f1", "mAP", "far",
...
def run(tr_imgs, tr_labels, te_imgs, te_labels, arms, seeds,
    ...
    for regime in REGIMES:
        ...
        teX, teY = build_multi(te_imgs, te_labels, n_test, seed=999,
                               ...
                               grid=grid, cell=cell)
        nX, _ = build_normal(n_normal, grid=grid, cell=cell)
        for arm in arms:
            for seed in seeds:
                ...
                print(f"[{regime:13s}] {arm:12s} s{seed} "
...
def print_table(rows):
    ...
    for regime in REGIMES:
        for arm in ARMS:
            bm, bs_ = _agg(rows, regime, arm, "bit_f1")
            mm, ms = _agg(rows, regime, arm, "mAP")
            fm, _ = _agg(rows, regime, arm, "far")
            nm, _ = _agg(rows, regime, arm, "normal_far")
...
def print_verdict(rows):
    ...
    for regime in REGIMES:
        ...
        bm, bstd = _agg(rows, regime, matched, "bit_f1")
        bo, bostd = _agg(rows, regime, other, "bit_f1")
        mm, _ = _agg(rows, regime, matched, "mAP")
        mo, _ = _agg(rows, regime, other, "mAP")
        win = bm > bo
        flip_ok = flip_ok and win
        print(f"[{regime}] matched={matched} bit_F1={bm:.4f} vs "
              f"{other} bit_F1={bo:.4f} | mAP {mm:.4f} vs {mo:.4f} "
              f"-> matched {'WINS' if win else 'does NOT win'}")
    print(f"\nFLIP {'APPEARS' if flip_ok else 'does NOT cleanly appear'}: "
          "matched operator best in each regime = "
          f"{flip_ok}")
